// Kanban view + reusable KanbanBoard
const { useState: useStateKb, useMemo: useMemoKb, useRef: useRefKb } = React;

// =============================================================================
// Group config
// =============================================================================
function getKanbanGroupConfig(projects) {
  function stageDot(s) {
    return {
      Discovery: "#A855F7", Evaluation: "#6366F1", Trial: "#F59E0B",
      Pilot: "#0EA5E9", Rollout: "#10B981", Paused: "#94A3B8"
    }[s] || "#94A3B8";
  }
  ...
  return {
    stage: { label: "Stage", keys: window.gtmStages, color: (k) => ({ bg: "#F1F5F9", text: "#0F172A", dot: stageDot(k) }) },
    priority: { label: "Priority", keys: ["high", "medium", "low"], color: (k) => ({
      high:   { bg: "#E0E7FF", text: "#3730A3", dot: "#4F46E5" },
      medium: { bg: "#E2E8F0", text: "#334155", dot: "#64748B" },
      low:    { bg: "#F1F5F9", text: "#64748B", dot: "#94A3B8" }
    }[k]) },
    rag: { label: "RAG status", keys: ["green","amber","red","unknown"], color: (k) => ({
      green:   { bg: "#ECFDF5", text: "#047857", dot: "#10B981" },
      amber:   { bg: "#FFFBEB", text: "#B45309", dot: "#F59E0B" },
      red:     { bg: "#FFF1F2", text: "#BE123C", dot: "#F43F5E" },
...
window.getKanbanGroupConfig = getKanbanGroupConfig;
...
// =============================================================================
// KanbanBoard — reusable column grid (no header chrome)
// Props:
//   projects: filtered project list to render
//   allProjects: full list (for figuring out auto column sets if grouping by owner/category)
//   groupBy: "stage" | "priority" | "rag" | "owner" | "category"
//   manual: { [groupKey::value]: [projectId] }  — manual ordering map
//   setManual: setter
//   canDrag: boolean
//   onCardClick: (project) => void
//   hideEmptyColumns: boolean
// =============================================================================
function KanbanBoard({ projects, allProjects, groupBy, manual, setManual, canDrag, onCardClick, hideEmptyColumns, onMoveProject, fields }) {
  const cfg = useMemoKb(() => getKanbanGroupConfig(allProjects || projects), [allProjects, projects, groupBy]);
  // owner can drag-reorder within any column now
  const supportsManual = canDrag;
  ...
  }, [projects, groupBy, manual, supportsManual, cfg, hideEmptyColumns]);

  const [dragId, setDragId] = useStateKb(null);
  ...
  function onDropOnCard(e, targetCol, targetPid) {
    ...
    moveCard(dragId, targetCol, targetPid);
    setDragId(null);
  }
  function onDropOnColumn(e, targetCol) {
    ...
    e.preventDefault();
    if (!dragId) return;
    moveCard(dragId, targetCol, null);
    setDragId(null);
  }
  function moveCard(pid, targetCol, beforeId) {
    ...
    if (currentVal !== targetCol && onMoveProject) {
      onMoveProject(pid, { [fieldKey]: targetCol });
    }
    if (!supportsManual) return;
      ...
      {columns.map(col => (
        <div key={col.key}
          ...
          <header className="px-3 py-2.5 flex items-center justify-between gap-2 border-b border-slate-200">
            ...
            {col.isManual && (
              <button className="text-[11px] text-slate-500 hover:text-indigo-600 inline-flex items-center gap-1" onClick={() => resetManual(col.key)} title="Reset to auto-sort">
...
// =============================================================================
// KanbanView — the built-in Kanban tab
// =============================================================================
function KanbanView({ fixedGroupBy, title } = {}) {
  const { projects, setRoute, updateProject, identity } = useStore();
  const [groupBy, setGroupBy] = useStateKb(fixedGroupBy || "stage");
  const [menuOpen, setMenuOpen] = useStateKb(false);
  const menuRef = useRefKb(null);
  useClickOutside(menuRef, () => setMenuOpen(false));

  // card field preferences (personal, persisted per browser)
  const [fields, setFields] = useStateKb(() => {
    try { const v = localStorage.getItem("gtm.kanbanFields"); return v ? JSON.parse(v) : window.gtmDefaultCardFields; } catch { return window.gtmDefaultCardFields; }
  });
  React.useEffect(() => { try { localStorage.setItem("gtm.kanbanFields", JSON.stringify(fields)); } catch {} }, [fields]);

  // manual ordering — persisted per browser, keyed by groupBy
  const [manualByGroup, setManualByGroup] = useStateKb(() => {
    try { const v = localStorage.getItem("gtm.kanbanManual"); return v ? JSON.parse(v) : {}; } catch { return {}; }
  });
  React.useEffect(() => { try { localStorage.setItem("gtm.kanbanManual", JSON.stringify(manualByGroup)); } catch {} }, [manualByGroup]);
  ...
  React.useEffect(() => { if (fixedGroupBy) setGroupBy(fixedGroupBy); }, [fixedGroupBy]);

  const canDrag = identity.isOwner;
  const cfg = getKanbanGroupConfig(projects);

  return (
    <div className="max-w-[1400px] mx-auto px-6 py-8" data-screen-label={title ? "06 " + title : "02 Kanban"}>
      <div className="mb-5 flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-1">
            {canDrag
              ? "Drag a card to reorder it inside a column or move it between columns. Manual order locks per column — hit Reset to revert."
                ...
                <div className="menu" style={{ top: 36, right: 0 }}>
                  {Object.entries(cfg).map(([k, c]) => (
                    <button key={k} className="menu-item" onClick={() => { setGroupBy(k); setManual({}); setMenuOpen(false); }}>
                      {groupBy === k ? <Icon name="check" size={13} className="text-indigo-600" /> : <span style={{ width: 13 }}></span>}
                      {c.label}
                    </button>
                  ))}
                </div>
...
// =============================================================================
// Card fields picker — popover with checkboxes
// =============================================================================
function CardFieldsPicker({ fields, onChange, label = "Card fields" }) {
  const [open, setOpen] = useStateKb(false);
  const ref = useRefKb(null);
  useClickOutside(ref, () => setOpen(false));
  function toggle(id) {
    onChange(fields.includes(id) ? fields.filter(x => x !== id) : [...fields, id]);
  }
  function reset() { onChange(window.gtmDefaultCardFields); }
  return (
      ...
      {open && (
        <div className="menu" style={{ top: 36, right: 0, width: 240, padding: 0 }}>
          <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between">
            ...
            <button onClick={reset} className="text-[11px] text-indigo-600 hover:underline">Reset</button>
          </div>
          <div className="p-2 flex flex-col gap-0.5 max-h-[50vh] overflow-y-auto">
            {window.gtmCardFields.map(f => (
...
function KanbanCard({ p, draggable, dragging, onDragStart, onDragOver, onDrop, onClick, fields }) {
  const r = window.ragMeta[p.rag];
  const pr = window.priorityMeta[p.priority];
  const f = fields || window.gtmDefaultCardFields;
  ...
  return (
    <article
      ...
      onDrop={onDrop}
      onClick={onClick}
      className={"card-hover bg-white rounded-lg shadow-card p-3 flex flex-col gap-2 " + (dragging ? "opacity-50 " : "") + (draggable ? "" : "cursor-pointer")}
    ...
    >
      ...
      {hasMidChips && (
        <div className="flex flex-wrap items-center gap-1">
          ...
          {has("decision") && p.decision && (
            <span className="text-[10px] px-1.5 py-0.5 rounded font-medium bg-indigo-50 text-indigo-700 border border-indigo-100">Decision</span>
          )}
        </div>
      )}
      {has("target") && (
        <div className="text-[11px] text-slate-500 inline-flex items-center gap-1.5">
          <Icon name="calendar" size={11} /> {window.fmtDate(p.target)}
        </div>
      )}
      {hasFooter && (
...
window.CardFieldsPicker = CardFieldsPicker;

window.KanbanView = KanbanView;
window.KanbanBoard = KanbanBoard;
window.KanbanCard = KanbanCard;
window.groupKeyLabel = groupKeyLabel;
