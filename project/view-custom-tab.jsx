// Custom Tab view — saved view with view-type, filters, group-by/sort
// Notion/Jira/Trello style: view toggle, filter chips, group-by, sort, all inline-editable
const { useState: useStateCt, useMemo: useMemoCt, useRef: useRefCt } = React;

// =============================================================================
// Normalize tab shape (back-compat with v1 tabs)
// =============================================================================
function normalizeCustomTab(tab) {
  return {
    ...
    cardFields: tab.cardFields || window.gtmDefaultCardFields,
    manual: tab.manual || {},
    filters: {
...
function activeFilterCount(f) {
  ...
  if (f.focusOnly) n++; if (f.decisionOnly) n++; if (f.stalledOnly) n++;
  if (f.search) n++;
  return n;
}

// =============================================================================
// Main custom tab view
// =============================================================================
function CustomTabView({ tab }) {
  const { projects, setRoute, updateProject, updateCustomTab, identity } = useStore();
  const t = useMemoCt(() => normalizeCustomTab(tab), [tab]);

  // persist manual ordering on the tab itself, keyed by groupBy
  ...
  const filtered = useMemoCt(() => applyFilters(projects, t.filters), [projects, t.filters]);
  const cfg = useMemoCt(() => window.getKanbanGroupConfig(projects), [projects]);
  const filterCount = activeFilterCount(t.filters);

  return (
    <div className="max-w-[1400px] mx-auto px-6 py-8" data-screen-label={"06 " + t.label}>
      {/* Title */}
      ...
      {/* Active filter chips */}
      {filterCount > 0 && (
        <ActiveFilters t={t} updateFilters={updateFilters} clearAllFilters={clearAllFilters} />
      )}

      {/* Board */}
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl shadow-card p-12 text-center mt-4">
          <div className="inline-flex h-10 w-10 rounded-full bg-slate-100 grid place-items-center text-slate-400 mb-2">
          ...
          {filterCount > 0 && (
            <button className="btn-ghost mt-3 mx-auto" onClick={clearAllFilters}><Icon name="x" size={12} /> Clear all filters</button>
          )}
        </div>
      ) : t.view === "kanban" ? (
        <KanbanBoard
          ...
          manual={manual}
          setManual={setManual}
          canDrag={identity.isOwner}
          hideEmptyColumns={t.hideEmptyColumns}
          fields={t.cardFields}
...
// =============================================================================
// Toolbar — view toggle + group/sort + filter button + delete
// =============================================================================
function Toolbar({ tab, update, updateFilters, clearAllFilters, filterCount, cfg, canEdit }) {
  const [filterOpen, setFilterOpen] = useStateCt(false);
  const filterRef = useRefCt(null);
  useClickOutside(filterRef, () => setFilterOpen(false));

  const [search, setSearch] = useStateCt(tab.filters.search || "");
  React.useEffect(() => { setSearch(tab.filters.search || ""); }, [tab.id]);
    ...
    <div className="bg-white rounded-xl shadow-card p-2 mb-3 flex items-center gap-2 flex-wrap">
      ...
      {tab.view === "kanban" ? (
        <PillSelect
          icon="layers"
          label="Group"
          value={cfg[tab.groupBy]?.label || tab.groupBy}
      ...
      {/* Card fields picker — shared component from view-kanban.jsx */}
      {canEdit && window.CardFieldsPicker && React.createElement(window.CardFieldsPicker, {
        fields: tab.cardFields,
        onChange: (v) => update({ cardFields: v }),
        label: tab.view === "kanban" ? "Card fields" : "Columns"
      })}
      ...
      <div className="relative" ref={filterRef}>
        ...
        {filterOpen && (
          <FilterPopover tab={tab} updateFilters={updateFilters} clearAllFilters={clearAllFilters} onClose={() => setFilterOpen(false)} canEdit={canEdit} />
        )}
      </div>
      ...
      <div className="inline-flex items-center gap-1.5 h-8 px-2.5 rounded-md border border-slate-200 bg-white text-[12px]">
        ...
        />
        {search && canEdit && (
          <button onClick={() => { setSearch(""); updateFilters({ search: "" }); }} className="text-slate-400 hover:text-rose-500">
            <Icon name="x" size={11} />
          </button>
        )}
      </div>

      {/* Hide empty (kanban only) */}
        ...
        <div className="menu" style={{ top: 36, left: 0, minWidth: 180 }}>
          {options.map(o => (
            <button key={o.value} className="menu-item" onClick={() => { onChange(o.value); setOpen(false); }}>
              {value === o.label ? <Icon name="check" size={13} className="text-indigo-600" /> : <span style={{ width: 13 }}></span>}
              {o.label}
            </button>
          ))}
        </div>
...
// =============================================================================
// Filter popover — multi-select for category/owner/priority/rag/stage + toggles
// =============================================================================
function FilterPopover({ tab, updateFilters, clearAllFilters, onClose, canEdit }) {
  const { projects } = useStore();
  const owners = useMemoCt(() => [...new Set(projects.map(p => p.owner).filter(Boolean))].sort(), [projects]);
  const categories = useMemoCt(() => [...new Set(projects.map(p => p.category).filter(Boolean))].sort(), [projects]);
  const f = tab.filters;
  ...
  return (
    <div className="menu" style={{ top: 40, right: 0, width: 360, padding: 0 }}>
      <div className="p-3 border-b border-slate-100 flex items-center justify-between">
        ...
        <button onClick={clearAllFilters} disabled={activeFilterCount(f) === 0} className="text-[11px] text-indigo-600 hover:underline disabled:text-slate-300 disabled:no-underline">Clear all</button>
      </div>
      ...
      <div className="p-3 flex flex-col gap-3 max-h-[60vh] overflow-y-auto nice-scroll">
        ...
        <FilterGroup label="Priority" values={["high","medium","low"]} selected={f.priorities} onToggle={(v) => toggle("priorities", v)} renderLabel={(v) => window.priorityMeta[v].label}
          renderSwatch={(v) => <span className="h-1.5 w-1.5 rounded-full" style={{ background: window.priorityMeta[v].text }}></span>}
          canEdit={canEdit} />

        <FilterGroup label="RAG status" values={["green","amber","red","unknown"]} selected={f.rags} onToggle={(v) => toggle("rags", v)} renderLabel={(v) => window.ragMeta[v].label}
          renderSwatch={(v) => <span className="h-1.5 w-1.5 rounded-full" style={{ background: window.ragMeta[v].dot }}></span>}
          canEdit={canEdit} />

        <FilterGroup label="Stage" values={window.gtmStages} selected={f.stages} onToggle={(v) => toggle("stages", v)} canEdit={canEdit} />
...
function ActiveFilters({ t, updateFilters, clearAllFilters }) {
  ...
  f.priorities.forEach(v => chips.push({ key: "p-" + v, label: "Priority: " + window.priorityMeta[v].label, clear: () => updateFilters({ priorities: f.priorities.filter(x => x !== v) }) }));
  f.rags.forEach(v => chips.push({ key: "r-" + v, label: "RAG: " + window.ragMeta[v].label, dot: window.ragMeta[v].dot, clear: () => updateFilters({ rags: f.rags.filter(x => x !== v) }) }));
  ...
  return (
    <div className="flex items-center gap-1.5 flex-wrap mb-3 text-[11.5px]">
      ...
      {chips.length > 1 && (
        <button onClick={clearAllFilters} className="text-indigo-600 hover:underline ml-1">Clear all</button>
      )}
    </div>
  );
}
...
function TableView({ projects, sort, fields, onCardClick }) {
  const visibleFields = fields || window.gtmDefaultCardFields;
  const has = (k) => visibleFields.includes(k);
  const sorted = useMemoCt(() => {
    const list = projects.slice();
    if (sort === "score") list.sort((a, b) => b.score - a.score);
    else if (sort === "name") list.sort((a, b) => a.name.localeCompare(b.name));
    else if (sort === "target") list.sort((a, b) => {
      if (a.target === "TBD") return 1;
      if (b.target === "TBD") return -1;
      return (a.target || "z").localeCompare(b.target || "z");
    });
    else if (sort === "rag") list.sort((a, b) => window.ragMeta[a.rag].severity - window.ragMeta[b.rag].severity);
            ...
            return (
              <tr key={p.id} onClick={() => onCardClick(p)} className="border-b border-slate-100 hover:bg-slate-50 cursor-pointer">
                <td className="px-4 py-2.5 text-center">
                  {p.focus ? <Icon name="star" size={13} className="text-amber-500 inline-block" /> : <span className="text-slate-200">·</span>}
                </td>
                ...
                {has("stage") && <td className="px-3 py-2.5">
                  <span className="text-[11px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-700 font-medium">{p.stage}</span>
                </td>}
                {has("rag") && <td className="px-3 py-2.5">
                ...
                {has("priority") && <td className="px-3 py-2.5">
                ...
                {has("target") && <td className="px-3 py-2.5 text-slate-600">{window.fmtDate(p.target)}</td>}
...
window.CustomTabView = CustomTabView;
window.applyFilters = applyFilters;
window.normalizeCustomTab = normalizeCustomTab;
