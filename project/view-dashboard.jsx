// Dashboard view — portfolio stats + filter chips + project grid
const { useMemo: useMemoDash, useState: useStateDash, useRef: useRefDash } = React;
...
function DashboardView() {
  ...
  const [showAdd, setShowAdd] = useStateDash(false);
  ...
  const filtered = useMemoDash(() => {
    ...
    list.sort(window.dashboardSort);
    return list;
  }, [projects, filter]);
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-6 py-8" data-screen-label="01 Dashboard">
      <div className="mb-6 flex items-end justify-between gap-6 flex-wrap">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-1">{projects.length} active {projects.length === 1 ? "project" : "projects"} · {window.fmtWeekLabel(new Date()).toLowerCase()} · <span className="inline-flex items-center gap-1">{viewMode === "exec" ? <><Icon name="briefcase" size={11} />Exec view</> : <><Icon name="wrench" size={11} />Ops view</>}</span></p>
        </div>
        <div className="hidden md:flex items-center gap-2 text-[12px] text-slate-500">
          <Icon name="calendar" size={13} />
          <span>Reporting cycle: <span className="text-[#0B0D12] font-medium">{window.fmtWeekRange(new Date())}</span></span>
        </div>
      </div>
  ...
  return (
    <article
      ...
      <footer className="flex items-center justify-between text-[11px] text-slate-500 pt-2 border-t border-slate-100 mt-1">
        <span className="inline-flex items-center gap-1.5"><Icon name="calendar" size={11} />{window.fmtDate(p.target)}</span>
...
function AddProjectModal({ onClose }) {
  ...
  const [category, setCategory] = useStateDash("Sales tooling");
  const [owner, setOwner] = useStateDash("Shivani");
  const [sponsor, setSponsor] = useStateDash("");
  const [target, setTarget] = useStateDash("");
  const [businessCase, setBusinessCase] = useStateDash("");
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="text-[12px] font-medium text-slate-700">
              ...
              <select className="bare mt-1 w-full h-9 px-2.5 rounded-md border border-slate-200 text-[13px]" value={category} onChange={e => setCategory(e.target.value)}>
                {window.gtmCategories.map(c => <option key={c}>{c}</option>)}
              </select>
            </label>
            <label className="text-[12px] font-medium text-slate-700">
              Owner
          ...
          <label className="text-[12px] font-medium text-slate-700">
            ...
            <textarea rows="3" value={businessCase} onChange={e => setBusinessCase(e.target.value)}
              className="mt-1 w-full p-2.5 rounded-md border border-slate-200 text-[13px]" placeholder="Why does this matter? What outcome are we after?"></textarea>
          </label>
          {!identity.isOwner && (
