...
function ProjectDetailInner({ project }) {
  ...
  const [confirmDelete, setConfirmDelete] = useStateP(false);
  ...
  function patchScoring(p) {
    ...
    const score = window.computeScore(next);
    updateProject(project.id, { scoring: next, score });
  }

  const r = window.ragMeta[project.rag];
  const pr = window.priorityMeta[project.priority];

  return (
    <div className="max-w-[1100px] mx-auto px-6 py-8" data-screen-label="03 Project detail">
      ...
      <div className="bg-white rounded-xl shadow-card p-6 mb-5">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[12px] text-slate-500 mt-2 flex flex-wrap items-center gap-x-3 gap-y-1">
              <span className="inline-flex items-center gap-1.5">
                ...
                <FieldPicker value={project.category} options={window.gtmCategories} canEdit={canEdit} onChange={(v) => patch({ category: v })} />
              </span>
              <span className="inline-flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-2">
            {canEdit ? (
              <button
                onClick={() => patch({ focus: !project.focus })}
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2">
          <PickerChip label="Stage" value={project.stage} options={window.gtmStages} canEdit={canEdit}
            renderValue={(v) => <span className="font-medium">{v}</span>}
            onChange={(v) => patch({ stage: v })} />
          <PickerChip label="RAG" value={project.rag} options={window.gtmRags} canEdit={canEdit}
            renderValue={(v) => {
              const meta = window.ragMeta[v];
              return <span className="inline-flex items-center gap-1.5 font-medium" style={{ color: meta.chipText }}>
                <span className="h-1.5 w-1.5 rounded-full" style={{ background: meta.dot }}></span>{meta.label}
              </span>;
            }}
            renderOption={(v) => {
              const meta = window.ragMeta[v];
              return <span className="inline-flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full" style={{ background: meta.dot }}></span>{meta.label}</span>;
            }}
            chipBg={r.chipBg}
            onChange={(v) => patch({ rag: v })} />
          <PickerChip label="Priority" value={project.priority} options={window.gtmPriorities} canEdit={canEdit}
            renderValue={(v) => <span className="font-medium" style={{ color: window.priorityMeta[v].text }}>{window.priorityMeta[v].label}</span>}
            renderOption={(v) => window.priorityMeta[v].label}
            chipBg={pr.bg}
            onChange={(v) => patch({ priority: v })} />
          <span className="inline-flex items-center gap-1.5 h-7 px-2.5 rounded-md bg-slate-50 border border-slate-200 text-[12px] text-slate-700" title="Computed from scoring sliders below">
          ...
          {canEdit && (
            <PickerChip label="Decision" value={project.decision ? "yes" : "no"} options={["yes", "no"]} canEdit={canEdit}
              renderValue={(v) => v === "yes"
                ? <span className="inline-flex items-center gap-1 font-medium text-indigo-700"><Icon name="git-pull-request-draft" size={11} />Needed</span>
                : <span className="text-slate-500 font-medium">None</span>}
              renderOption={(v) => v === "yes" ? "Needs decision" : "No decision pending"}
              chipBg={project.decision ? "#EEF2FF" : "#fff"}
              onChange={(v) => patch({ decision: v === "yes" })} />
          )}
        ...
        <div className="mt-4">
          ...
          <StakeholderChips
            ...
            canEdit={canEdit}
            onChange={(v) => patch({ stakeholders: v })}
          />
        </div>
      </div>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Editable tag="div" canEdit={canEdit}
                  value={project.askText || (canEdit ? "Click to add the ask…" : "No outstanding ask.")}
                  onCommit={(v) => patch({ askText: v })}
                  className="text-[13px] text-indigo-900/90 leading-relaxed" />
              </div>
              <div>
                <div className="text-[11px] uppercase tracking-wider text-indigo-700/80 font-medium mb-1">Shivani's recommendation</div>
                <Editable tag="div" canEdit={canEdit}
                  value={project.askRecommendation || (canEdit ? "Click to add a recommendation…" : "—")}
                  onCommit={(v) => patch({ askRecommendation: v })}
                  className="text-[13px] text-indigo-900/90 leading-relaxed" />
              </div>
            </div>
          </div>
      ...
      {/* operational detail — Ops view only */}
      {viewMode === "ops" && (
        <div className="bg-white rounded-xl shadow-card p-6 mb-5">
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-[11px] text-slate-500 inline-flex items-center gap-1.5">
              ...
              Sliders compute the priority score above
            </div>
          </div>
        ...
        <div className="modal-backdrop" onClick={() => setConfirmDelete(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-2 mt-5">
              ...
              <button className="btn-ghost danger" onClick={() => deleteProject(project.id)}>
                <Icon name="trash-2" size={13} /> Delete project
              </button>
            </div>
          </div>
        </div>
...
function DateChip({ value, canEdit, onChange }) {
  ...
  return (
    <span className={"inline-flex items-center gap-1.5 h-7 px-2.5 rounded-md bg-slate-50 border border-slate-200 text-[12px] text-slate-700 " + (canEdit ? "cursor-pointer hover:bg-slate-100" : "")} onClick={openPicker}>
      ...
      <span className="font-medium">{window.fmtDate(value)}</span>
      {canEdit && (
        <input type="date" ref={ref} value={iso}
          onChange={(e) => onChange(e.target.value || "TBD")}
          className="sr-only pointer-events-none" tabIndex="-1" aria-hidden="true" />
      )}
...
function DateField({ value, canEdit, onChange }) {
  const iso = value && value !== "TBD" && /^\d{4}-\d{2}-\d{2}$/.test(value) ? value : "";
  if (!canEdit) {
    return <span className="text-slate-700">{window.fmtDate(value)}</span>;
  }
  return (
    <div className="flex items-center gap-2">
      <input type="date" value={iso}
        onChange={(e) => onChange(e.target.value || "TBD")}
        className="h-8 px-2.5 rounded-md border border-slate-200 text-[13px] bg-white" />
      {iso && (
        <button onClick={() => onChange("TBD")} className="text-[11px] text-slate-400 hover:text-rose-500 inline-flex items-center gap-1">
...
function Editable({ tag = "div", value, canEdit, onCommit, className = "" }) {
  ...
  // use suppressContentEditableWarning + dangerouslySet via children
  if (tag === "h1") return <h1 ref={ref} contentEditable={canEdit} suppressContentEditableWarning className={cls}
    onBlur={(e) => canEdit && onCommit(e.target.innerText)}>{value}</h1>;
  if (tag === "span") return <span ref={ref} contentEditable={canEdit} suppressContentEditableWarning className={cls}
    onBlur={(e) => canEdit && onCommit(e.target.innerText)}>{value}</span>;
  return <div ref={ref} contentEditable={canEdit} suppressContentEditableWarning className={cls}
    onBlur={(e) => canEdit && onCommit(e.target.innerText)}>{value}</div>;
}
        ...
        <div className="menu" style={{ top: 22, left: 0, minWidth: 180 }}>
          {options.map(o => (
            <button key={o} className="menu-item" onClick={() => { onChange(o); setOpen(false); }}>
              {value === o ? <Icon name="check" size={13} className="text-indigo-600" /> : <span style={{ width: 13 }}></span>}
              {o}
            </button>
          ))}
        </div>
    ...
    <div className="flex flex-wrap items-center gap-1.5">
      {items.map(s => (
        <span key={s} className="inline-flex items-center gap-1.5 h-7 pl-1.5 pr-2 rounded-full bg-slate-100 text-slate-700 text-[12px] group">
          <span className="h-5 w-5 rounded-full bg-gradient-to-br from-slate-300 to-slate-400 text-white text-[9px] grid place-items-center font-semibold">
            {s.split(" ").map(x => x[0]).slice(0,2).join("")}
          </span>
          {s}
          {canEdit && (
            <button onClick={() => onChange(items.filter(x => x !== s))} className="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-rose-500 -mr-0.5">
              <Icon name="x" size={12} />
            </button>
          )}
        </span>
      ...
      {canEdit && (
        <span className="inline-flex items-center">
          <input value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); add(); } if (e.key === "Backspace" && !draft && items.length) onChange(items.slice(0, -1)); }}
    ...
    <div className="flex flex-col gap-1.5">
      {items.map((it, i) => (
        <div key={i} className="flex items-start gap-2 group">
          <span className="h-5 w-5 rounded-md border border-slate-300 grid place-items-center text-slate-300 hover:text-indigo-500 cursor-pointer mt-0.5">
            <Icon name="check" size={11} />
          </span>
          <span
            contentEditable={canEdit} suppressContentEditableWarning
            onBlur={(e) => { if (!canEdit) return; const next = items.slice(); next[i] = e.target.innerText; onChange(next); }}
            className={"flex-1 text-[13px] text-slate-700 " + (canEdit ? "editable" : "")}
          >{it}</span>
          {canEdit && (
            <button onClick={() => onChange(items.filter((_, idx) => idx !== i))} className="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-rose-500">
              <Icon name="x" size={13} />
            </button>
          )}
        </div>
...
function BucketList({ title, tone, items, canEdit, onChange }) {
  const toneMap = {
    rose:   { dot: "#F43F5E", bg: "#FFF1F2", text: "#9F1239" },
    amber:  { dot: "#F59E0B", bg: "#FFFBEB", text: "#92400E" },
    indigo: { dot: "#6366F1", bg: "#EEF2FF", text: "#3730A3" }
  };
...
function Slider({ label, value, onChange, canEdit, sub }) {
  return (
    <div>
      ...
      <input type="range" min="0" max="10" step="1" value={value} disabled={!canEdit}
...
function DecisionsCard({ project }) {
  ...
  function add() {
    ...
    const newDec = {
      ...
      actor: identity.name, text: text.trim()
    };
    updateProject(project.id, { decisions: [newDec, ...(project.decisions || [])] });
    setText("");
  }
  function remove(id) {
    updateProject(project.id, { decisions: (project.decisions || []).filter(d => d.id !== id) });
  }
  function edit(id, newText) {
    updateProject(project.id, { decisions: (project.decisions || []).map(d => d.id === id ? { ...d, text: newText } : d) });
  }

  return (
    <section className="bg-white rounded-xl shadow-card p-5">
      <h3 className="text-[13px] font-semibold tracking-tight mb-3 inline-flex items-center gap-1.5">
        <Icon name="git-branch" size={13} className="text-slate-400" /> Decisions log
      </h3>
          ...
          return (
            <div key={d.id} className="text-[12px] flex gap-2 group">
              ...
              <div className="flex-1 min-w-0">
                <span contentEditable={canModify} suppressContentEditableWarning
                  onBlur={(e) => canModify && edit(d.id, e.target.innerText)}
                  className={"text-slate-700 " + (canModify ? "editable" : "")}>{d.text}</span>
                <span className="text-slate-400"> — {d.actor}</span>
              </div>
              {canModify && (
                <button onClick={() => remove(d.id)} className="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-rose-500 shrink-0 self-start mt-0.5">
                  <Icon name="x" size={12} />
                </button>
              )}
            </div>
...
function CommentsCard({ project }) {
  ...
  function remove(id) {
    updateProject(project.id, { comments: (project.comments || []).filter(c => c.id !== id) });
  }
  function edit(id, newText) {
    updateProject(project.id, { comments: (project.comments || []).map(c => c.id === id ? { ...c, text: newText } : c) });
  }

  return (
    <section className="bg-white rounded-xl shadow-card p-5">
      <h3 className="text-[13px] font-semibold tracking-tight mb-3 inline-flex items-center gap-1.5">
        <Icon name="message-square" size={13} className="text-slate-400" /> Comments
      </h3>
        ...
        {(project.comments || []).map((c) => {
          ...
          return (
            <div key={c.id} className="flex items-start gap-2 group">
              <div className="h-6 w-6 rounded-full bg-gradient-to-br from-indigo-300 to-violet-400 text-white text-[10px] grid place-items-center font-semibold shrink-0">
                {c.who.split(" ").map(x => x[0]).slice(0,2).join("")}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[13px] text-slate-700 mt-0.5">
                  <span contentEditable={canModify} suppressContentEditableWarning
                    onBlur={(e) => canModify && edit(c.id, e.target.innerText)}
                    className={canModify ? "editable" : ""}>{c.text}</span>
                </div>
              </div>
              {canModify && (
                <button onClick={() => remove(c.id)} className="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-rose-500 self-start mt-1">
                  <Icon name="x" size={12} />
                </button>
              )}
            </div>
